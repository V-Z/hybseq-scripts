...
while getopts "hrvf:c:o:d:q:a:j:m:t:" INITARGS; do
	case "$INITARGS" in
		h) # Help and exit
			...
			echo -e "\t-c\tNumber of CPU threads to use for parallel operations. If not provided, default is 2."
...
if [ -z "$NCPU" ]; then
	echo "Number of CPU threads (-c) for parallel operations was not set. Using default value of 15."
	echo
	NCPU='15'
	fi
